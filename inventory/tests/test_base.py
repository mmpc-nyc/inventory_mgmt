...
class AbstractTest(TestCase):
    ...
    def setUp(self):
        ...
        self.equipment_stored_working = Equipment.objects.create(name='stored_working', product=self.product,
                                                                 condition=self.condition_working, stock=self.stock,
                                                                 location=self.stock.location

                                                                 )
        self.equipment_picked_up_working_1 = Equipment.objects.create(name='picked_up_working_1', product=self.product,
                                                                      condition=self.condition_working, stock=self.stock,
                                                                      location=self.location_user, user=self.user,
                                                                      status=Equipment.Status.PICKED_UP)
        self.equipment_picked_up_working_2 = Equipment.objects.create(name='picked_up_working_2', product=self.product,
                                                                      condition=self.condition_working, stock=self.stock,
                                                                      location=self.location_user, user=self.user,
                                                                      status=Equipment.Status.PICKED_UP)
        self.equipment_deployed_working_1 = Equipment.objects.create(name='deployed_working_1', product=self.product,
                                                                     condition=self.condition_working, stock=self.stock,
                                                                     status=Equipment.Status.DEPLOYED)
        self.equipment_deployed_working_2 = Equipment.objects.create(name='deployed_working_2', product=self.product,
                                                                     condition=self.condition_working, stock=self.stock,
                                                                     status=Equipment.Status.DEPLOYED)

        self.order_deploy_initial = Order.objects.create(
            activity= Order.Activity.DEPLOY,
            customer= self.customer_1,
            location= self.customer_1_location_1.location,
            date=timezone.now(),
        )
        self.order_deploy_partial = Order.objects.create(
            activity=Order.Activity.DEPLOY,
            customer=self.customer_1,
            location=self.customer_1_location_1.location,
            date=timezone.now(),
        )
        self.order_deploy_complete = Order.objects.create(
            activity=Order.Activity.DEPLOY,
            customer=self.customer_1,
            location=self.customer_1_location_1.location,
            date=timezone.now(),
        ...
        self.order_inspect = Order.objects.create(
            activity= Order.Activity.INSPECT,
            customer= self.customer_1,
            location= self.customer_1_location_1.location,
            date=timezone.now()
        )
