...
(function () {
    ...
    function U(a, b) {
        ...
        for (b = {text: "", bytes: [], chunks: [], version: b}; 4 <= a.available();) {
            var d = a.readBits(4);
            if (d === t.Terminator) return b;
            if (d === t.ECI) 0 === a.readBits(1) ? b.chunks.push({type: r.ECI, assignmentNumber: a.readBits(7)}) :
                ...
                }) : b.chunks.push({type: r.ECI, assignmentNumber: -1}); else if (d === t.Numeric) {
                var e = a;
                d = [];
                for (var f = "", g = e.readBits([10, 12, 14][c]); 3 <= g;) {
                    var h = e.readBits(10);
                    if (1E3 <= h) throw Error("Invalid numeric value above 999");
                    var k = Math.floor(h / 100), n = Math.floor(h / 10) % 10;
                    h %= 10;
    ...
    function W(a, b, c, d) {
        ...
        for (var f = a.one; c.degree() >= d / 2;) {
            ...
            for (g = a.inverse(g); c.degree() >= b.degree() && !c.isZero();) {
                let d = c.degree() - b.degree(), e = a.multiply(c.getCoefficient(c.degree()), g);
    ...
    function X(a, b) {
        ...
        a = new Y(285,
            256, 0);
        var d = new w(a, c), e = new Uint8ClampedArray(b), f = !1;
        for (var g = 0; g < b; g++) {
        ...
        b = d[0];
        g = b.degree();
        if (1 === g) b = [b.getCoefficient(1)]; else {
            e = Array(g);
            f = 0;
    ...
    function Z(a, b, c) {
        c = aa[c.dataMask];
        let d = a.height;
        var e = 17 + 4 * b.versionNumber, f = A.createEmpty(e, e);
        f.setRegion(0, 0, 9, 9, !0);
        f.setRegion(e - 8, 0, 8, 9, !0);
        f.setRegion(0, e - 8, 9, 8, !0);
        for (var g of b.alignmentPatternCenters) for (var h of b.alignmentPatternCenters) 6 ===
    ...
    function ba(a) {
        var b = a.height, c = Math.floor((b - 17) / 4);
        if (6 >= c) return K[c -
        1];
        c = 0;
        for (var d = 5; 0 <= d; d--) for (var e = b - 9; e >= b - 11; e--) c = C(a.get(e, d), c);
        d = 0;
        for (e = 5; 0 <= e; e--) for (let c = b - 9; c >= b - 11; c--) d = C(a.get(e, c), d);
        a = Infinity;
        let f;
        for (let e of K) {
    ...
    function ea(a, b, c) {
        let d = b.errorCorrectionLevels[c], e = [], f = 0;
        ...
        if (1 < d.ecBlocks.length) for (g = d.ecBlocks[0].numBlocks, b = d.ecBlocks[1].numBlocks, c = 0; c < b; c++) e[g + c].codewords.push(a.shift());
    ...
    function ha(a, b) {
        var c = fa({x: 3.5, y: 3.5}, {x: b.dimension - 3.5, y: 3.5}, {
                x: b.dimension - 6.5,
                y: b.dimension - 6.5
            }, {x: 3.5, y: b.dimension - 3.5}), d = M(b.topLeft, b.topRight, b.alignmentPattern, b.bottomLeft),
            e = d.a11 * c.a11 + d.a21 * c.a12 + d.a31 * c.a13, f = d.a12 * c.a11 +
                d.a22 * c.a12 + d.a32 * c.a13, g = d.a13 * c.a11 + d.a23 * c.a12 + d.a33 * c.a13,
            h = d.a11 * c.a21 + d.a21 * c.a22 + d.a31 * c.a23, k = d.a12 * c.a21 + d.a22 * c.a22 + d.a32 * c.a23,
            n = d.a13 * c.a21 + d.a23 * c.a22 + d.a33 * c.a23, m = d.a11 * c.a31 + d.a21 * c.a32 + d.a31 * c.a33,
            l = d.a12 * c.a31 + d.a22 * c.a32 + d.a32 * c.a33, p = d.a13 * c.a31 + d.a23 * c.a32 + d.a33 * c.a33;
        c = A.createEmpty(b.dimension, b.dimension);
        d = (a, b) => {
            const c = g * a + n * b + p;
            return {x: (e * a + h * b + m) / c, y: (f * a + k * b + l) / c}
        };
        for (let e = 0; e < b.dimension; e++) for (let f = 0; f < b.dimension; f++) {
            let b = d(f + .5, e + .5);
    ...
    function ia(a, b, c) {
        let d = y(a, b), e = y(b, c), f = y(a, c), g, h, k;
    ...
    function N(a, b, c, d) {
        ...
        let k = Math.abs(a - g), n = Math.abs(b - h), m = Math.floor(-k / 2), l = g < a ? 1 : -1, p = h < b ? 1 : -1,
            q = !0;
        for (let v = g, u = h; v !== a + l; v += l) {
            g = f ? u : v;
            h = f ? v : u;
            if (c.get(g,
                h) !== q && (q = !q, e.push({x: g, y: h}), e.length === d + 1)) break;
            m += n;
            if (0 < m) {
                if (u === b) break;
                u += p;
    ...
    function O(a, b, c) {
        try {
            let d = z(a, {x: -1, y: a.y}, c, b.length),
                ...
                f = z(a, {x: Math.max(0, a.x - a.y) - 1, y: Math.max(0, a.y - a.x) - 1}, c, b.length),
                g = z(a, {x: Math.min(c.width, a.x + a.y) + 1, y: Math.min(c.height, a.y + a.x) + 1}, c, b.length),
                h = F(d, b), k = F(e, b), n = F(f, b), m = F(g, b),
    ...
    function ka(a) {
        ...
        for (var m of b) 2 > m.bottom.y - m.top.y || (b = (m.top.startX + m.top.endX + m.bottom.startX + m.bottom.endX) / 4, e = (m.top.y + m.bottom.y + 1) / 2, a.get(Math.round(b), Math.round(e)) && (f = [m.top.endX - m.top.startX, m.bottom.endX - m.bottom.startX, m.bottom.y - m.top.y + 1], f = x(f) / f.length, g = O({
            x: Math.round(b),
            y: Math.round(e)
        }, [1, 1, 3, 1, 1], a), c.push({score: g, x: b, y: e, size: f})));
        ...
        for (b = 0; b < Math.min(c.length, 5); ++b) {
            ...
            for (var l of c) l !== e && f.push(Object.assign(Object.assign({},
                l), {score: l.score + Math.pow(l.size - e.size, 2) / e.size}));
            f.sort((a, b) => a.score - b.score);
            m.push({points: [e, f[0], f[1]], score: e.score + f[0].score + f[1].score})
        }
    ...
    function P(a, b, c, d, e) {
        ...
        var h = c.x - d.x + e.x, k = c.y - d.y + e.y;
        c = (y(d, e) + y(d, c)) / 2 / g;
        e = 1 - 3 / c;
    ...
    function Q(a) {
        ...
        for (let e of b) {
            ...
            if (null == c) c = null; else {
                var d = L(c);
                if (d) c = d; else {
                    for (d = 0; d < c.width; d++) for (let a = d + 1; a < c.height; a++) c.get(d, a) !== c.get(a, d) && (c.set(d, a, !c.get(d, a)), c.set(a, d, !c.get(a, d)));
                    c = L(c)
                }
            }
            if (c) return {
                binaryData: c.bytes, data: c.text, chunks: c.chunks, version: c.version,
                location: {
    ...
    function I(a, b, c, d = {}) {
        ...
        var g = e.greyScaleWeights, h = e.canOverwriteImage, k = b * c;
        ...
        m = new S(b, c, m);
        if (g.useIntegerApproximation) for (var l = 0; l < c; l++) for (var p = 0; p < b; p++) {
            var q = 4 * (l * b + p);
            m.set(p, l, g.red * a[q] + g.green * a[q + 1] + g.blue * a[q + 2] + 128 >> 8)
        } else for (l = 0; l < c; l++) for (p = 0; p < b; p++) q = 4 *
        ...
        v = new S(g, l, v);
        for (p = 0; p < l; p++) for (q = 0; q < g; q++) {
            var u = Infinity, r = 0;
            for (var t = 0; 8 > t; t++) for (let a = 0; 8 > a; a++) {
        ...
        for (b = 0; b < l; b++) for (a = 0; a < g; a++) {
            c = g - 3;
            c = 2 > a ? 2 : a > c ? c : a;
            h = l - 3;
            h = 2 > b ? 2 : b > h ? h : b;
            k = 0;
            for (n = -2; 2 >= n; n++) for (u = -2; 2 >= u; u++) k += v.get(c + n, h + u);
            c = k / 25;
            for (h = 0; 8 > h; h++) for (k = 0; 8 > k; k++) n = 8 * a + h, u = 8 * b + k, r = m.get(n, u), p.set(n, u, r <= c), f && q.set(n, u, !(r <= c))
        }
    ...
    class A {
        ...
        setRegion(a, b, c, d, e) {
            for (let f = b; f < b + d; f++) for (let b = a; b < a + c; b++) this.set(b, f, !!e)
        }
    }
    ...
    class w {
        constructor(a, b) {
            ...
            if (1 < c && 0 === b[0]) {
                let d = 1;
                for (; d < c && 0 === b[d];) d++;
                if (d === c) this.coefficients = a.zero.coefficients; else for (this.coefficients = new Uint8ClampedArray(c - d), a = 0; a < this.coefficients.length; a++) this.coefficients[a] = b[d + a]
        ...
        addOrSubtract(a) {
            ...
            let c = new Uint8ClampedArray(a.length), d = a.length - b.length;
        ...
        multiply(a) {
            ...
            if (1 === a) return this;
            let b = this.coefficients.length, c = new Uint8ClampedArray(b);
        ...
        multiplyPoly(a) {
            ...
            let b = this.coefficients, c = b.length;
            a = a.coefficients;
            let d = a.length, e = new Uint8ClampedArray(c + d - 1);
            for (let h = 0; h < c; h++) {
                let c = b[h];
                for (let b = 0; b < d; b++) {
                    var f = h + b, g = this.field.multiply(c, a[b]);
                    e[f] = e[h + b] ^ g
                }
            }
    ...
    let K = [{
        ...
        },
            {
                ...
                infoBits: null, versionNumber: 4, alignmentPatternCenters: [6, 26], errorCorrectionLevels: [{
                    ecCodewordsPerBlock: 20, ecBlocks: [{
                        numBlocks: 1,
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 26, ecBlocks: [{numBlocks: 4, dataCodewordsPerBlock: 13},
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 26, ecBlocks: [{numBlocks: 4, dataCodewordsPerBlock: 14},
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 24, ecBlocks: [{numBlocks: 4, dataCodewordsPerBlock: 12},
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 28, ecBlocks: [{
                        numBlocks: 6,
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 24, ecBlocks: [{
                        numBlocks: 3,
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 20, ecBlocks: [{numBlocks: 11, dataCodewordsPerBlock: 16},
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 28, ecBlocks: [{numBlocks: 10, dataCodewordsPerBlock: 46},
                ...
                infoBits: 79154, versionNumber: 19, alignmentPatternCenters: [6, 30, 58, 86], errorCorrectionLevels: [{
                    ecCodewordsPerBlock: 28, ecBlocks: [{numBlocks: 3, dataCodewordsPerBlock: 113}, {
                        numBlocks: 4,
                ...
                infoBits: 84390, versionNumber: 20, alignmentPatternCenters: [6, 34, 62, 90], errorCorrectionLevels: [{
                    ecCodewordsPerBlock: 28, ecBlocks: [{
                        numBlocks: 3,
                ...
                infoBits: 87683, versionNumber: 21, alignmentPatternCenters: [6, 28, 50, 72, 94], errorCorrectionLevels: [{
                    ecCodewordsPerBlock: 28,
                ...
                infoBits: 92361, versionNumber: 22, alignmentPatternCenters: [6, 26, 50, 74, 98], errorCorrectionLevels: [{
                    ecCodewordsPerBlock: 28,
                ...
                infoBits: 96236, versionNumber: 23, alignmentPatternCenters: [6, 30, 54, 74, 102], errorCorrectionLevels: [{
                    ecCodewordsPerBlock: 30, ecBlocks: [{
                        numBlocks: 4,
            ...
            },
            {
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 30, ecBlocks: [{numBlocks: 33, dataCodewordsPerBlock: 16},
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 30, ecBlocks: [{numBlocks: 4, dataCodewordsPerBlock: 24}, {
                        numBlocks: 31,
                ...
                errorCorrectionLevels: [{
                    ...
                    ecCodewordsPerBlock: 28, ecBlocks: [{numBlocks: 19, dataCodewordsPerBlock: 47},
        ...
        y = (a, b) => Math.sqrt(Math.pow(b.x - a.x, 2) + Math.pow(b.y - a.y, 2)), la = {
            inversionAttempts: "attemptBoth",
            greyScaleWeights: {red: .2126, green: .7152, blue: .0722, useIntegerApproximation: !1},
            canOverwriteImage: !0
        };
    I.default = I;
    let G = "dontInvert", D = {red: 77, green: 150, blue: 29, useIntegerApproximation: !0};
    self.onmessage = a => {
        let b = a.data.data;
        switch (a.data.type) {
            case "decode":
                a = I(b.data, b.width, b.height, {inversionAttempts: G, greyScaleWeights: D});
